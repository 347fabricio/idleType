...
let upgradePrice = document.querySelectorAll(".keyboardUpgrade");
let price = document.querySelectorAll(".product-price");
let ex = document.querySelectorAll(".product");
let products = Array.from({ length: ex.length });
...
// ==============================================
const getTax = (x, y) => {
  storeProducts[y].cost += Math.ceil(storeProducts[y].cost * (Math.floor(Math.random() * 4) + 1));
};
...
const getQuantity = () => {
  ...
  for (let i = 1; i < storeProducts.length; i++) {
    sum += storeProducts[i].quantity;
  }
...
const buyWriter = (x, y) => {
  if (keyboards >= storeProducts[y].cost) {
    keyboards -= storeProducts[y].cost;
    showKeyboards.innerText = keyboards.toFixed(1);
    storeProducts[y].quantity += 0.2;
    storeProducts[y].level += 1;
    sum = getQuantity();
    moneyPerSecond.innerText = `money/second: ${sum.toFixed(1)}`;
    getTax(x, y);
...
let upgradesList = [
  {
    ...
    // bought: 0,
    currentUpgrade: 0,
    cost: [70, 500, 7000, 70000],
    upgradeIndex: Array.from(upgradePrice),
  },
];

const boost = () => {
  const x = storeProducts[0].level;
  switch (true) {
    case x == 0:
      upgradesList[0].upgradeIndex[0].style.display = "flex";
      console.log("take it.");
      break;
    case x == 1:
      upgradesList[0].upgradeIndex[1].style.display = "flex";
      // storeProducts[0].quantity += 1;
      console.log("hello, dude.");
      break;
    case x == 5:
      // storeProducts[0].quantity += 10;
      console.log("great job!");
      break;
    case x == 10:
      // storeProducts[0].quantity += 25;
...
const buyUpgrade = (x, y) => {
  if (keyboards >= upgradesList[0].cost[y]) {
    ...
    alert(`upgrade ${Number(y) + 1} cost ${upgradesList[0].cost[y]} keyboards.`, typeof y);
  }
};
